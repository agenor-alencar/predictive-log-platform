...
/**
 * Processador de Streams Kafka para Agregação em Tempo Real.
 * 
 * Teoria para aula:
 * - Janelas de Tempo (Tumbling Windows): O Kafka Streams agrupa os logs em blocos de 
 *   5 minutos. Isso permite calcular médias e contagens sem precisar ler todo o 
 *   banco de dados, processando tudo "no voo" (in-flight).
 * - Agregação: Transforma logs brutos em estatísticas (counts, médias) e envia 
 *   o resultado para um novo tópico 'plip.logs.aggregated'.
 */
@Component
@Slf4j
public class LogStreamProcessor {
        ...
        public void buildTopology(StreamsBuilder streamsBuilder) {
                ...
                KStream<String, String> processedStream = rawLogs
                                .groupByKey(Grouped.with(Serdes.String(), Serdes.String()))
                                .windowedBy(TimeWindows.ofSizeWithNoGrace(Duration.ofMinutes(5)))
                                .aggregate(
                                                // Initializer: { "count": 0, "total_response_time": 0.0, "error_count":
                                                // 0 }
                                                ...
                                                (key, value, aggregate) -> {
                                                        try {
                                                                ...
                                                                double totalRt = aggNode.get("total_response_time")
                                                                                .asDouble()
                                                                                + logNode.path("responseTimeMs")
                                                                                                .asDouble(0.0);
                                                                long errors = aggNode.get("error_count").asLong()
                                                                                + (logNode.path("statusCode")
                                                                                                .asInt(200) >= 400 ? 1
                                                                                                                : 0);
                                ...
                                .map((windowedKey, value) -> {
                                        try {
                                                ...
                                                log.info("Window aggregation: method={}, count={}, avgRT={}ms, errors={}",
                                                                windowedKey.key(), count, String.format("%.2f", avgRt),
                                                                errors);

                                                return KeyValue.<String, String>pair(windowedKey.key(), result);
                                        } catch (Exception e) {
                                                log.error("Window result mapping error: {}", e.getMessage());
                                                return KeyValue.<String, String>pair(windowedKey.key(), value);
                                        }
                                });

                processedStream.to(KafkaConfig.LOGS_AGGREGATED_TOPIC, Produced.with(Serdes.String(), Serdes.String()));
        }
}
